...
class ProfileHeaderView: UIView {
    ...
    // Установка внутреннего размера компонента
        ...
        UIView.animateKeyframes(
            ...
            animations: {
                UIView.addKeyframe(withRelativeStartTime: 0.0, relativeDuration: 0.5) {
                    self.avatarImageView.center = CGPoint(x: centerX, y: centerY)
                    self.avatarImageView.transform = CGAffineTransform(scaleX: newSize, y: newSize)
                ...
                print("Did finish Open animation")
            })
    }
    ...
    private func closeAnimationExample() {
        ...
        UIView.animate(
            ...
            self.avatarImageView.center = CGPoint(x: center, y: center)
            self.avatarImageView.layer.cornerRadius = 65
            self.avatarImageView.layer.borderWidth = 3
            self.avatarImageView.transform = CGAffineTransform.identity
        } completion: { _ in
            print("Did finish Close animation")
        }
    }
    // MARK: - Настройка профиля
